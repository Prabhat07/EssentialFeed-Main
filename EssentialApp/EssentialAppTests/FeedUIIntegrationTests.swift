...
class FeedUIIntegrationTests: XCTestCase {
    ...
    func test_loadMoreActions_requestMoreFromLoader() {
        ...
        sut.simulateLoadMoreFeedAction()
        XCTAssertEqual(loader.loadMoreCallCount, 2, "Expected request when load more completed with more pages")
        
        loader.completeLoadMoreWithError(at: 1)
        sut.simulateLoadMoreFeedAction()
        XCTAssertEqual(loader.loadMoreCallCount, 3, "Expected request while load more failure")
    ...
    func test_loadMoreIndicator_isVisibleWhileLaodingMore() {
        let (sut, loader) = makeSUT()
        ...
        sut.simulateLoadMoreFeedAction()
        XCTAssertTrue(sut.isShowingLoadMoreFeedIndicator, "Expected loading indicator once user initiates a load more")
        
        loader.completeLoadMore(at: 0)
        XCTAssertFalse(sut.isShowingLoadMoreFeedIndicator, "Expected no loading indicator once user completes with success")
        
        sut.simulateLoadMoreFeedAction()
        XCTAssertTrue(sut.isShowingLoadMoreFeedIndicator, "Expected loading indicator on second load more action")
        
        loader.completeLoadMoreWithError(at: 1)
        XCTAssertFalse(sut.isShowingLoadMoreFeedIndicator, "Expected no loading indicator once user completes with a error")
    }
    ...
    func test_laodFeedCompletion_dispatchesFromBackgroundToMainThread() {
        let (sut, loader) = makeSUT()
        sut.simulateAppearance()
    ...
    func test_laodMoreCompletion_dispatchesFromBackgroundToMainThread() {
        let (sut, loader) = makeSUT()
        sut.simulateAppearance()
        loader.completeFeedLoading(at: 0)
        sut.simulateLoadMoreFeedAction()
    ...
    func test_feedImageViewIndicator_isVisibleWhileLoadingImage() {
        let (sut, loader) = makeSUT()
    ...
    func test_feedImageView_doseNotRenderLoadedImageWhenNotVisibleAnymore() {
        let (sut, loader) = makeSUT()
        ...
        XCTAssertNil(view?.renderedImage, "Expected no image renderd when image load finishes after the view is not visible anymore")
    }
    
    func test_laodImageDataCompletion_dispatchesFromBackgroundToMainThread() {
        let (sut, loader) = makeSUT()
        sut.simulateAppearance()
