...
//
//  Created by Prabhat Tiwari on 13/04/21.
//
...
class EssentialFeedEndToEndTests: XCTestCase {
    ...
    func test_endToEndTestForMatchesFixedResponse_fromTestServer() {
        ...
        case let .success(imageFeed)?:
            XCTAssertEqual(imageFeed.count, 8, "Expected 8 images items in response image feed")
    ...
    private func getFeedImageDataResult(file: StaticString = #file, line: UInt = #line) -> FeedImageDataLoader.Result? {
        ...
        client.get(from: url) { result in
            receivedResult = result.flatMap { (data, respones) in
                do {
                    return .success(try FeedImageDataMapper.map(data, respones))
                } catch {
                    return .failure(error)
                }
            }
